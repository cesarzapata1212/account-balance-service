...
public class AccountRepository {
    ...
    public void insert(String accountNumber, String sortCode) throws SQLException {
        Integer count = new JdbcSession(dataSource)
                .sql("INSERT INTO account (account_number, sort_code) VALUES (?, ?)")
                .set(accountNumber)
                .set(sortCode)
    ...
    public boolean exists(String accountNumber, String sortCode) throws SQLException {
        Long count = new JdbcSession(dataSource)
                .sql("SELECT COUNT(1) FROM account WHERE account_number = ? AND sort_code = ?")
                .set(accountNumber)
                .set(sortCode)
